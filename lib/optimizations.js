...
function enableECPubKeyCaching () {
  ...
  utils.sharedEncryptionKey = function (aPriv, bPub) {
    if (typeof aPriv !== 'string' || typeof bPub !== 'string') {
      return sharedEncryptionKey.apply(this, arguments)
    }

    var key = aPriv + bPub
    var cached = cache1.get(key)
    if (!cached) {
      cached = sharedEncryptionKey.apply(this, arguments)
      cache1.set(key, cached)
    }
