...
var DEFAULT_PLUGINS = [plugins.sigcheck, plugins.prev, plugins.identity];
...
module.exports = function chainstream(options) {
  ...
  return combine.obj(map(function (txEntry, done) {
    var processed = extend({}, txEntry);
    // var tx = bitcoin.Transaction.fromBuffer(txEntry.tx)
    chainloader.load(txEntry).then(function (obj) {
  ...
  }), mkParser(), basicIdCheck(), verify(verifier));
