...
function checkFrom (chainedObj) {
  ...
  if (!(chainedObj.from && chainedObj.to)) return

  // at this point, from/to are based on tx addresses
  // if possible, take them from signing party
  // not possible if an object was shared that was authored by a third party
  var objAuthor = chainedObj.parsed.data[SIGNEE]
  if (!objAuthor) return

  var objAuthorRH = objAuthor && objAuthor.split(':')[0]
  var txAuthorRH = chainedObj.from[ROOT_HASH]
  var toRH = chainedObj.to[ROOT_HASH]
  if (toRH === objAuthorRH) {
    // switch
    var tmp = chainedObj.from
    chainedObj.from = chainedObj.to
    chainedObj.to = tmp
  }
}
