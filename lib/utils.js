...
var utils = module.exports = {
  getMsgProps: function getMsgProps(info) {
    return {
  ...
  validateMsg: function validateMsg(msg) {
    try {
  ...
  msgToBuffer: function msgToBuffer(msg) {
  ...
  bufferToMsg: function bufferToMsg(buf) {
  ...
  parseUID: function parseUID(uid) {
    var parts = uid.split('-');
    var parsed = {};
  ...
  toErrInstance: function toErrInstance(err) {
  ...
  setUID: function setUID(entry) {
    utils.setEntryProp(entry, 'uid', utils.getUID(entry));
  },

  firstKey: function firstKey(keys, where) {
  ...
  getDHTKey: function getDHTKey(obj, cb) {
    if (!obj[NONCE]) return Q.reject(new Error('missing nonce'));

    var b = new Builder().data(obj);
  ...
  addError: function addError(opts) {
    typeforce({
      group: 'String',
      entry: 'Object',
      error: 'Object'
  ...
  getErrors: function getErrors(entry, group) {
    if (!(group in Errors.group)) {
      throw new Error('invalid error group');
    }
  ...
  countErrors: function countErrors(entry, group) {
    if (group) {
  ...
  updateEntry: function updateEntry(older, newer) {
    ...
    return Entry.fromJSON(olderJSON).set(newerJSON).set('timestamp', olderJSON.timestamp).set('errors', mergedErrs).set('timesProcessed', timesProcessed);
  },

  now: Date.now.bind(Date),

  toObj: function toObj() /* k1, v1, k2, v2... */{
    var obj = {};
  ...
  errToJSON: function errToJSON(err) {
  ...
  toBuffer: function toBuffer(data, enc) {
    if (typeof data.toBuffer === 'function') return data.toBuffer();
    if (Buffer.isBuffer(data)) return data;
    if ((typeof data === 'undefined' ? 'undefined' : _typeof(data)) === 'object') data = tutils.stringify(data);

    return new Buffer(data, enc || 'binary');
  },

  getEntryProp: function getEntryProp(entry, prop) {
    return entry instanceof Entry ? entry.get(prop) : entry[prop];
  },

  setEntryProp: function setEntryProp(obj, name, val) {
    if (obj instanceof Entry) obj.set(name, val);else obj[name] = val;
  },

  getOTRKeyFingerprint: function getOTRKeyFingerprint(identity) {
    return find(identity.pubkeys, function (k) {
      return k.type === 'dsa' && k.purpose === 'sign';
    }).fingerprint;
  },

  keyForFingerprint: function keyForFingerprint(identityJSON, fingerprint) {
    return find(identityJSON.pubkeys, function (k) {
      return k.fingerprint === fingerprint;
    });
  },

  updateChainedObj: function updateChainedObj(obj, prevHash) {
    obj[ROOT_HASH] = obj[ROOT_HASH] || prevHash;
    obj[PREV_HASH] = prevHash;
    return obj;
  },

  filterStream: function filterStream(filter) {
   ...
   */
  rateLimitPromiseFn: function rateLimitPromiseFn(fn, millis) {
    var lastCalled = 0;
    return function () {
      var now = utils.now();
      ...
      return utils.promiseDelay(delay).then(call);
  ...
  jsonifyErrors: function jsonifyErrors(entry) {
    var errs = utils.getEntryProp(entry, 'errors');
    if (errs) {
  ...
  parseCommonBlockchainTxs: function parseCommonBlockchainTxs(txInfos) {
  ...
  delay: function delay(millis) {
    return regeneratorRuntime.async(function delay$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          _context.next = 2;
          return regeneratorRuntime.awrap(Q.Promise(function (resolve) {
            setTimeout(resolve, millis);
          }));
