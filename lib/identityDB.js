...
module.exports = function mkIdentityDB (path, options) {
  ...
  function processEntry (entry, cb) {
    if (topics.indexOf(entry.get('type')) === -1) return cb()

    if (entry.get(TYPE) !== IDENTITY_TYPE) {
      return cb()
    }
    ...
    db.get(prefixKey(CUR_HASH, curHash), function (err, val) {
      if (err){
        ...
        if (!err.notFound) {
          debug('database choked on get: ' + JSON.stringify(err))
        }
    ...
    function save () {
      return keeper.getOne(curHash)
        .then(doSave, function (err) {
          if (err instanceof TypeError ||
            err instanceof ReferenceError) {
            throw err
          }

          debug('unable to get identity from keeper', curHash, err)
          cb()
        })
    }
    ...
    function doSave (buf) {
      ...
      batch.forEach(function (item, i) {
        db.get(item.key, function (err, result) {
      ...
      var togo = batch.length

      function next () {
        if (--togo) return

        var conflict = results.some(function (r) {
          // only allow iterations of identity
          // by original identity publisher
          return r && r !== rootHash
        })

        if (conflict) {
          debug('ignoring identity that presents keys already presented by another identity')
          return cb()
        }
...
function toJSON (buf) {
  if (Buffer.isBuffer(buf)) buf = buf.toString()
