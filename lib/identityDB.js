...
module.exports = function mkIdentityDB(path, options) {
  ...
  db.createReadStream = function (options) {
    ...
    return createReadStream.call(db, extend(options, {
      ...
      if (options.keys !== false) {
        ...
        if (options.values === false) data = key;else data.key = key;
      }
      ...
      getContents(id, function (err, contents) {
        ...
        var ret = options.keys === false ? contents : {
          key: data.key,
          value: contents
        };
  ...
  db.query = function (query, cb) {
    ...
    if (Object.keys(query).length !== 1 || !(query.fingerprint || query[ROOT_HASH] || query[CUR_HASH])) {
  ...
  function getContents(id, cb) {
    log.get(id, function (err, entry) {
      ...
      keeper.getOne(curHash).then(function (buf) {
        var ret = {
          identity: toJSON(buf)
        };
  ...
  function processEntry(entry, cb) {
    if (entry.get(TYPE) !== Identity.TYPE || entry.get('type') !== EventType.chain.readSuccess) {
      return cb();
    }
