...
module.exports = function createMsgDB (path, options) {
  ...
  function processEntry (entry, cb) {
    ...
    switch (eType) {
      ...
      case EventType.msg.receivedInvalid:
        myDebug('received msg', entry.id(), entry.get(CUR_HASH), entry.get('txId'), entry.get('dateReceived'))
        ...
        return updateByCurHash({
          ...
          cb()
          // console.log(db.name, 'received', entry.get(CUR_HASH))
      ...
      case EventType.chain.writeSuccess:
        myDebug('msg chained', entry.get(CUR_HASH))
      ...
      case EventType.chain.readSuccess:
        ...
        return updateByCurHash({
          entry: entry,
          createIfMissing: true
        }, callbackWithEmit(cb, 'unchained'))
      case EventType.chain.readError:
        myDebug('msg unchained (error)', entry.get(CUR_HASH))
        // TODO:
      default:
        return cb()
    }
  ...
  //   // query.txId = entry.get('txId')

  //   collect(db.query(query), function (err, vals) {
  //     if (!db.isOpen()) return cb()

  //     if (err || !vals.length) {
  //       myDebug('not found', curHash)
  //       return putAndReturnVal(getKey(entry), entry.toJSON(), cb)
  //     }
  ...
  //     if (vals.length > 1) {
  //       return cb(
  //         new Error('invalid db state, multiple entries with the same txId and hash: ' +
  //           JSON.stringify(vals))
  //       )
  //     }
  ...
  //     myDebug('updating', curHash, stored.prev())
