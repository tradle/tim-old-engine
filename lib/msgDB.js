...
module.exports = function createMsgDB(path, options) {
  ...
  db.byCurHash = db.liveOnly(function (curHash, cb) {
    ...
    cb = safe(cb);
    var stream = db.createValueStream().on('data', function (value) {
  ...
  db.getConversation = function (otherGuyRootHash, raw, cb) {
    ...
    collect(pump(raw ? db.rawReadStream({ keys: false }) : db.createValueStream(), utils.filterStream(function (val) {
      var from = val.from && val.from[ROOT_HASH];
      var to = val.to && val.to[ROOT_HASH];
      return val[TYPE] !== TYPES.IDENTITY && (from === otherGuyRootHash || to === otherGuyRootHash);
    })), cb);
  };
  ...
  function update(entry, cb) {
    ...
    return db.get(key, function (err, state) {
      ...
      var newEntry = state ? utils.updateEntry(state, entry) : entry;
...
function addTimestampOps(state, update, batch) {
  ...
  if (utils.getEntryProp(state, 'chain')) {
    ...
    } else if (utils.getEntryProp(state, 'dateChained') || utils.getErrors(state, 'chain') >= Errors.MAX_CHAIN) {
      // remove from queue
      batch.push({
        type: 'del',
  ...
  if (utils.getEntryProp(state, 'deliver')) {
    ...
    } else if (utils.getEntryProp(state, 'dateSent') || utils.getErrors(state, 'send') >= Errors.MAX_SEND) {
      batch.push({
        type: 'del',
