  ...
  return regeneratorRuntime.async(function _callee2$(_context3) {
    while (1) switch (_context3.prev = _context3.next) {
      ...
      case 25:
        ...
        this._debug('waiting to for txs to get processed...', _context3.t1);
        _context3.next = 30;
        return regeneratorRuntime.awrap(utils.delay(Driver.CATCH_UP_INTERVAL));
...
Driver.prototype._setupDBs = function () {
  ...
  var msgDBEvents = ['chained', 'unchained', 'message', 'sent'];
...
Driver.prototype.receiveMsg = function (buf, senderInfo) {
  ...
  if (valid) promiseValid = Q.resolve();else promiseValid = Q.reject(new Error('received invalid msg'));

  var from;
  return this._receiving = promiseValid.then(this.lookupIdentity.bind(this, senderInfo)).then(function (result) {
    from = result;
    ...
    return Q.all([self.keeper.put(parsed.permissionKey.toString('hex'), msg.encryptedPermission), self.keeper.put(permission.fileKeyString(), msg.encryptedData)]);
  }).then(function () {
    // yes, it repeats work
    // but it makes the code simpler
    // TODO optimize
    return self.lookupObject(txInfo);
  }).then(self.unchainResultToEntry).then(function (entry) {
    return entry.set({
...
Driver.prototype.share = function _callee7(options) {
 ...
 */
Driver.prototype.send = function _callee8(options) {
...
Driver.prototype.history = function (identityInfo) {
  ...
  return lookupOtherParty.then(function (other) {
    ...
    if (other) {
      ...
      stream = pump(stream, utils.filterStream(function (data) {
        ...
        return data.from && data.from[ROOT_HASH] === otherPartyRootHash || data.to && data.to[ROOT_HASH] === otherPartyRootHash;
      }));
    }
